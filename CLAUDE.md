...
**DEC-024 (HTML dashboard render target) — COMPLETE 2026-06-24 (v5.0.0, MAJOR).** The dashboard is now a single read-only, offline, `file://`-openable **HTML** page (`.claude/dashboard.html`, gitignored/derived) generated by `dashboard-render.py --html`; all visualizations are inline SVG rendered in Python (zero runtime/CDN deps). **Markdown hard-retired** — the 14 MD renderers + `--render`/`--tasks-section` modes deleted; `--html`/`--task-hash` remain. Hand-rolled layered-DAG→SVG dependency graph (auto-hides when degenerate); 53-phase heatmap; decisions collapsed to a searchable link-out; spec linked-out; sidecar is the sole user-content source (no in-file markers; full-regen-only). Record `decisions/decision-024-html-dashboard-render-target.md` (`implemented`, 9 anchors); research archive `decisions/.archive/decision-024-research-2026-06-24.md`; per-ship detail in `ship-log.md` v5.0.0. 11 downstream dashboards auto-migrate via the `template_version` format-staleness trigger. **Caveat:** the DEC record + research mislabel the broken-mermaid bug as "FB-007" — that's wrong (real FB-007 = spec-edit guardrail, DEC-016/v4.0.0); no FB tracked mermaid. No open follow-ups.
...
- **FB-011** (special-case): scripts tracker — Families A+B shipped (v3.0.0/3.1.1), **Family E dropped 2026-05-20** (v4.6.2), **Family C CLOSED — full port shipped v4.22.0** (2026-06-11; PoC v4.19.0 → evidence run against styler's 218+620-task corpus → user "go"). **Superseded by DEC-024 (v5.0.0): the render target is now HTML** — `dashboard-render.py --html` owns the entire document (inline-SVG visualizations), the LLM fills the "Needs you" (Action Required) + Custom Views placeholders, `--task-hash` is the single hash authority; the `--render`/`--tasks-section` Markdown modes were deleted. 47 dashboard tests (24 data-layer + 23 HTML); record in `template-maintenance/scripts-candidates.md § Family C` + ship-log v4.22.0 / v5.0.0. Family D remains deferred (triggers on observed real LLM-missed parallel conflicts; note 2026-06-11: the styler T729/T731 package.json collision was an *input-data* miss — under-declared files_affected — not a check-logic miss, so the trigger stands unmet).
