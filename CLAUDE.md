...
- **FB-112** (open — three small independent sub-items, bundled per FB-006 precedent): (a) `template-maintenance/architecture-map.md` `Current as of: v5.1.1`, four versions stale — v5.4.0 added script→state derivation edges the map may not reflect; (b) `dashboard-regeneration.md:96–103` documents a dead `tasks` toggle (post-DEC-024 the renderer reads only `action_required`/`decisions`/`custom_views`/`notes`); (c) `implementation_anchors` shape inconsistent — 4 styles across 21 records, only DEC-021/022/023 match the prescribed schema. **Hazard when normalizing (c):** a plain YAML scalar cannot contain colon-space; that mistake invalidated four records' frontmatter mid-edit on 2026-08-12. **Note (v5.5.0):** sub-item (a)'s map is now five versions stale (v5.5.0 added a script → `scripts-candidates.md` records it; the map was not staged with the v5.5.0 commit per the FB-112 split) — reconciling it closes the pre-commit hook's warn-only topology nudge too.
