...
- **FB-103 … FB-108** (harvest Tier-2, 2026-07-19; triage: `template-maintenance/harvest-2026-07-19-triage.md`): **103 + 108 shipped v5.3.0 and archived** (limit-cutoff recovery protocol; owner:both personal-data verification shape). **104 cheap slice shipped v5.3.0** (pre-dispatch git re-check, handoff preserve-not-consume, single-committer conventions) — full concurrency model research-gated on recurrence. **106 doc half shipped v5.3.0** (pause new-section carve-out) — marker mechanism queued with **105** (AR-card auto-render; MINOR design, tinder prior art) as the next ship. **107** stays 2nd-signal-gated. Same date: FB-087 archived (helper gate never fired), FB-033 watch note added (declaration-entailment adjacent signal, styler 06-24).
