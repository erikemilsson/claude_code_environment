...
- **FB-103 … FB-108** (harvest Tier-2, 2026-07-19; triage: `template-maintenance/harvest-2026-07-19-triage.md`): **103, 105, 106, 108 shipped and archived** — limit-cutoff recovery + owner:both personal-data verification (v5.3.0); script-owned Action Required card + `pending_decomposition[]` marker (v5.4.0). **104 cheap slice shipped v5.3.0** (pre-dispatch git re-check, handoff preserve-not-consume, single-committer conventions) — **still open**: full concurrency model (session registry/lease) research-gated on recurrence. **107** stays 2nd-signal-gated (build-blocking vs validation phase gates). Housekeeping same window: FB-087 archived, FB-033 watch note, DEC-017 status flipped `approved`→`implemented` (8/9 anchors verified; 9th intentionally removed by DEC-020).
...
- **FB-109** (open — mechanization candidate, Family F): session-export inbox rename is prose-executed at three call sites (`context-transitions.md:411`, `/work pause` step 6, `/work` Step 0f) and **still violated months after its v4.21.2 patch** — 4 of 7 dot-prefixed exports found 2026-08-12 postdate the rule. Consumer side hardened in v5.4.3 (Part 7 enumeration now dotfile-inclusive) but that is a backstop, not the cure. Fix: extract the inbox copy-with-rename into a deterministic `.claude/scripts/` helper (`persist-friction.py` precedent); fires every pause in every downstream project, so it clears the scripts-candidates ROI bar.
- **FB-112** (open — three small independent sub-items, bundled per FB-006 precedent): (a) `template-maintenance/architecture-map.md` `Current as of: v5.1.1`, four versions stale — v5.4.0 added script→state derivation edges the map may not reflect; (b) `dashboard-regeneration.md:96–103` documents a dead `tasks` toggle (post-DEC-024 the renderer reads only `action_required`/`decisions`/`custom_views`/`notes`); (c) `implementation_anchors` shape inconsistent — 4 styles across 21 records, only DEC-021/022/023 match the prescribed schema. **Hazard when normalizing (c):** a plain YAML scalar cannot contain colon-space; that mistake invalidated four records' frontmatter mid-edit on 2026-08-12.
- **FB-113 … FB-117** (harvest 2026-08-12, 56 exports; plan + full cluster analysis: `template-maintenance/harvest-2026-08-12-plan.md`, status COMPLETE): all five **open, none shipped**. **113** `files_affected` under-counts in a new domain (cross-artifact doc↔notebook parity + dispatch-brief re-sync when a scope note post-dates the field) — unqualified `ready`, 9 incidents, extends FB-058/FB-086 and the 2026-06-11 predecessor whose deferral trigger is now met. **114** negative-findings positive-control rule defeated along three axes (invalid control, `Grep` absent from subagent sessions, and a shell-function `grep` that skips gitignored files) — unqualified `ready`, 9 incidents; sub-issue 2 is upstream-gated like FB-075/FB-077. **115** closure sweeps verify the phrasings you changed rather than the claim's surface; the fix is a structural task-ID cross-reference, not a wider vocabulary — single-project caveat. **116** verify-agent `runtime_validation` third failure mode + the local-ceiling-determination gap (agents both over- and under-claim what runs locally) — sibling extending FB-076, see that bullet. **117** task/AC authoring internal consistency at two authoring sites (ACs needing a *user* decision on `owner: claude` tasks; enumerated counts wrong at creation; circular phase-result↔fingerprint re-verification trigger) — single-project caveat.
...
**Decision record status (as of 2026-07-20):** 21 records; 19 `implemented` with anchors. `approved` exceptions — both correct as-is: DEC-003 (flat layout) + DEC-018 (command routing), research conclusions with no implementation commit. (DEC-017 flipped to `implemented` 2026-07-20.)
