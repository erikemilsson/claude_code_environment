...
- **FB-096** (A resolved v4.21.3; B decision-pending): model-surface refresh — (A) ✅ docs-verified 2026-06-11: capability doc now records aliases `sonnet|opus|haiku|fable` + full model IDs + `inherit` + resolution order + per-agent `effort:` frontmatter (drift was broader than the original `fable`-only capture). (B) pin re-evaluation remains Erik's call — ratify float / pin explicit (now CONFIRMED feasible, full IDs documented) / move pin forward. Verified facts in the FB entry's resolution note.
