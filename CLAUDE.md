...
- **FB-006** (ready, direct promotion): audit-findings dashboard/CLI workflow — courier pattern (dashboard tick → CLI re-specification), audit-name memory burden, dead `[Promote to FB] / [Dismiss]` inline text, opacity at decision moment (no plain-English description on dashboard). Three sequencing directions: render-time consolidation (drop dead inline text + add plain-English description per finding), `/audit-coherence triage` interactive walker (eliminates name-memory), dual-checkbox promote-vs-dismiss column. Likely ships as 2-3 audit-family iterations (PATCH drop-dead-UI → MINOR digest extension → MINOR triage command). Surface: `commands/audit-coherence.md` (digest synthesizer + possible `triage` subcommand), `rules/dashboard.md` + `support/reference/dashboard-regeneration.md`, `skills/dashboard-style/SKILL.md`, possibly `support/audits/<name>/digest.json` schema (`plain_english_description` field per finding). No decision conflicts (DEC-013 specified routing rules, not visual element shape). Precursor to deferred Audit family Stage 7.
