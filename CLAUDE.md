...
- **FB-006** (iterations 1+2+3 shipped — v3.17.1 PATCH + v3.18.0 MINOR + v3.19.0 MINOR; all four sub-issues closed, ready to archive on next `/feedback review`): audit-findings dashboard/CLI workflow. **Iteration 1 (PATCH, v3.17.1) shipped:** dropped per-finding `[Promote to FB] / [Dismiss]` inline text from dashboard render (closes sub-issue 3 "dead UI"); `[Fix it]` retained for bundle-eligible; non-bundle-eligible kinds show italicized kind annotation only. **Iteration 2 (MINOR, v3.18.0) shipped:** added `description` field to `digest.json items[]` (Component 2 schema) — plain-English synthesizer-written sentence per finding, rendered on the dashboard digest in place of the terse `title` (closes sub-issue 4 "opacity at decision moment"). Synthesizer prompts in `commands/audit-coherence.md` + `commands/audit-ui.md` updated (Algorithm step 4); render rules use `{description ?? title}` for backward-compat with older digest.json files. **Iteration 3 (MINOR, v3.19.0) shipped:** `/audit-coherence triage` + `/audit-ui triage` interactive walkers — `audit-ts` defaults to `latest` (newest by `ran_at`), user never types the audit name; walker iterates pending non-dismissed findings with kind-conditional actions `[F]ix it · [P]romote · [D]ismiss · [S]kip · [Q]uit`; dispatches to canonical per-action mechanics (no divergence); supports single-letter shorthand + natural-language verbs (`dismiss because X` parses reason inline). Closes sub-issue 1 (courier pattern: dashboard tick → CLI re-specification) + sub-issue 2 (audit-name memory burden). Cross-references added in `audit-fix-workflow.md` (top-of-action-protocol "preferred entry point" note) + `dashboard-regeneration.md` "How to act on findings" + `dashboard-style/SKILL.md` mirror. **Dual-checkbox promote-vs-dismiss column** (originally MINOR-or-deferred candidate): obsoleted by iteration 3's triage walker, which provides explicit per-action dispatch without relying on checkbox semantics. Close as obsoleted unless concrete need surfaces post-3.19.0. Precursor to deferred Audit family Stage 7 (bundled-apply batch UX) remains.
