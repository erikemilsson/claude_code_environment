# Scenario 07: Session Resumption
...
| Task status | `.claude/tasks/task-*.json` | `/work` Step 1, Step 3 |
| Decision status | `.claude/support/decisions/decision-*.md` | `/work` Step 2b |
| Inflection point revision | `spec_revised` field in decision frontmatter | `/work` Step 2b-post, `/iterate` |
| Dashboard | `.claude/dashboard.md` (+ META block hash) | `/work` Step 1a |
| Phase progress | Derived from task statuses | `/work` Step 3 |
| Drift state | `.claude/drift-deferrals.json` | `/work` Step 1b |

---

## 7A: Interrupted during `/iterate`
...
## 7B: Interrupted during `/work` (mid-implementation)

### State
- Task 3: "In Progress" in task JSON
- Some files partially modified

### Trace: `/work` Step 3 routing
...
## 7C: Interrupted between decision resolution and `/iterate`
...
### Key mechanism
`spec_revised` field is the durable checkpoint. No time-based heuristics.

---

## 7D: Interrupted between phases
