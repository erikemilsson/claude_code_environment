...
| F2 | Session-export inbox copy-with-rename — the `context-transitions.md:411` rename rule (NEVER copy the dot-prefixed working filename verbatim) executed deterministically instead of as prose at `/work pause` step 6 + `/work` Step 0f | `support/reference/context-transitions.md § "Session Export"` step 6, `commands/work.md § "Step 0f"` step 8 | N (writes the user-configured external inbox path, never `.claude/`) |
...
**F2 shipped (2026-08-12, FB-109):** `.claude/scripts/persist-session-export.py` (v5.5.0) mechanizes the inbox copy-with-rename that the prose rule at `context-transitions.md:411` kept failing under end-of-session context pressure (4 of 7 dot-prefixed inbox exports found 2026-08-12 postdate the v4.21.2 prose patch, one via the Step 0f recovery path the rule names explicitly). Parallel to the `persist-friction.py` complement above: F2 is the "documented but not executed" pattern mechanized at its specific call sites (pause step 6 + Step 0f step 8), distinct from F1's post-batch invariant *check*. The script writes the inbox copy itself (the one shipped script that writes a file — the destination is the external `template_inbox_path`, never `.claude/`, so DEC-004 is not violated) because handing the `cp` back to the orchestrator would leave the rename failure surface in place. 19 tests; the v5.4.3 consumer-side dotfile-inclusive enumeration in `/health-check` Part 7 remains as a backstop. F1 itself stays parked per its trigger.
